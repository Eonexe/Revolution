...
public class WurstplusArrayList extends WurstplusPinnable {
	...
	public void render() {
		...
		List<WurstplusHack> pretty_modules = Chungus.get_hack_manager().get_array_active_hacks().stream()
			.sorted(Comparator.comparing(modules -> get(modules.array_detail() == null ? modules.get_tag() : modules.get_tag() + Chungus.g + " [" + Chungus.r + modules.array_detail() + Chungus.g + "]" + Chungus.r, "width")))
			.collect(Collectors.toList());

		int count = 0;

		if (Chungus.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top R") || Chungus.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top L") ) {
		...
		for (WurstplusHack modules : pretty_modules) {
			...
			if (flag) {
				String module_name = (
					...
					modules.get_tag() + Chungus.g + " [" + Chungus.r + modules.array_detail() + Chungus.g + "]" + Chungus.r
				);

				if (Chungus.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Free")) {
					...
					if (Chungus.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top R")) {
					...
					if (Chungus.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top L")) {
						mc.fontRenderer.drawStringWithShadow(module_name, 2, 3 + count * 10, new WurstplusDraw.TravisColor(nl_r,nl_g,nl_b,nl_a).hex());
						count++;
					}
					if (Chungus.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Bottom R")) {
					...
					if (Chungus.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Bottom L")) {
