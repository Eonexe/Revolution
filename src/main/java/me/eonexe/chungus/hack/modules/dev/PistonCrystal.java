...
import me.eonexe.chungus.hack.event.events.WurstplusEventMove;
import me.eonexe.chungus.hack.event.events.WurstplusEventPacket;
import me.eonexe.chungus.hack.event.events.WurstplusEventPacket.ReceivePacket;
import me.eonexe.chungus.hack.guiscreen.settings.WurstplusSetting;
import me.eonexe.chungus.hack.manager.WurstplusModuleManager;
import me.eonexe.chungus.hack.modules.WurstplusCategory;
import me.eonexe.chungus.hack.modules.WurstplusHack;
import me.eonexe.chungus.hack.util.WurstplusPlayerUtil;
import me.eonexe.chungus.hack.util.WurstplusRotationUtil;
...
public class PistonCrystal extends WurstplusHack {
	...
	WurstplusSetting breakType = create("Type", "paType", "Swing", combobox("Swing", "Packet"));
	WurstplusSetting placeMode = create("Place", "paPlace", "Torch", combobox("Torch", "Block", "Both"));
	WurstplusSetting target = create("Tartet", "paTarget", "Nearest", combobox("Nearest", "Looking"));
	WurstplusSetting enemyRange = create("enemyRange", "paRange", 4.9F, 0.0F, 6.0F);
	WurstplusSetting crystalDeltaBreak = create("Center Break", "paBreak", 0.1F, 0.0F, 0.5F);
	WurstplusSetting crystalPlaceTry = create("Crystal Place Try", "paPlaceTry", 15, 2, 30);
	WurstplusSetting blocksPerTick = create("Blocks Per Tick", "paBlocks", 4, 0, 20);
	WurstplusSetting minHealth = create("Min Health", "paHealth", 8, 0, 20);
	WurstplusSetting supBlocksDelay = create("Surrond Delay", "paSupport", 4, 0, 20);
	WurstplusSetting preRotationDelay = create("Pre Rotation Delay", "paPreDelay", 0, 0, 20);
	WurstplusSetting afterRotationDelay = create("Post Rotation Delay", "paPostDelay", 0, 0, 20);
	WurstplusSetting startDelay = create("Start Delay", "paStartDelay", 4, 0, 20);
	WurstplusSetting pistonDelay = create("Piston Delay", "paPDelay", 2, 0, 20);
	WurstplusSetting crystalDelay = create("Crystal Delay", "paCDelay", 2, 0, 20);
	WurstplusSetting redstoneDelay = create("Redstone Delay", "paRDelay", 0, 0, 20);
	WurstplusSetting midHitDelay = create("Mid Hit Delay", "paMHDelay", 5, 0, 20);
	WurstplusSetting hitDelay = create("Hit Delay", "paHDelay", 2, 0, 20);
	WurstplusSetting stuckDetector = create("Stuck Check", "paCheck", 35, 0, 200);
	WurstplusSetting maxYincr = create("Max Y", "paMaxY", 3, 0, 5);
	WurstplusSetting blockPlayer = create("Trap Player", "paTrap", true);	
	WurstplusSetting rotate = create("Rotate", "paPotate", false);
	WurstplusSetting confirmBreak = create("No Glich Break", "paGlitchBreak", true);
	WurstplusSetting confirmPlace = create("No Glitch Place", "paGlichPlace", true);
	WurstplusSetting allowCheapMode = create("Cheap Mode", "paCheap", false);
	WurstplusSetting betterPlacement = create("Better Place", "paBetterPlace", true);
	WurstplusSetting bypassObsidian = create("Bypass Obsidian", "paBypass", false);
	WurstplusSetting antiWeakness = create("Anti Weakness", "paWeakness", false);
	WurstplusSetting debugMode = create("Debug Mode", "paDebug", false);
	WurstplusSetting speedMeter = create("Speed Meter", "paSpeed", false);
	WurstplusSetting packetReducer = create("Packet Reducer", "paReducer", false);
	WurstplusSetting preRotation = create("Pre Rotate", "paPreRotate", false);
	WurstplusSetting forceRotation = create("Force Rotate", "paForceRotate", false);
	WurstplusSetting chatMsg = create("Chat MSGs", "paChat", true);
    ...
    private boolean getAimTarget() {
        ...
        if (target.in("Nearest"))
            aimTarget = WurstplusPlayerUtil.findClosestTarget(enemyRange.getValue(), aimTarget);
            // if looking
        else
            aimTarget = WurstplusPlayerUtil.findLookingPlayer(enemyRange.getValue());
    ...
    @EventHandler
    private final Listener<ReceivePacket> packetReceiveListener = new Listener<>(event -> {
    ...
    private void initValues() {
        ...
        //if (WurstplusModuleManager.isModuleEnabled(AutoCrystal.class)) {
    ...
    @SuppressWarnings("unused")
    @EventHandler
    private final Listener<WurstplusEventMove> onUpdateWalkingPlayerEventListener = new Listener<>(event -> {
        ...
        PlayerPacket packet = new WurstplusPlayerPacket(this, rotation);
    ...
    public void update() {
        ...
        WurstplusRotationUtil.shouldSpoofAngles(true);
