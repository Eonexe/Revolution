package me.eonexe.chungus.hack.modules.chat;
...
public class WurstplusChatSuffix extends WurstplusHack {
	...
	private Listener<WurstplusEventPacket.SendPacket> listener = new Listener<>(event -> {
		...
		if (accept_suffix) {
			if (suffix_default) {
				// Just default.
				message += Chungus.WURSTPLUS_SIGN + convert_base("wurstplus two");
			}
			...
			if (suffix_random) {
				...
				message += Chungus.WURSTPLUS_SIGN + suffix_with_randoms.toString(); 
			}
